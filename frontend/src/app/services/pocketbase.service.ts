...
import { CreateOfferDTO, PublicOfferDTO, ReadOfferDTO } from '../model/offer';
...
export class PocketbaseService {
	...
	public async GetPublicOfferByID(id: string): Promise<PublicOfferDTO> {

		const o = await this.pb.collection('public_offers').getOne(id)

		let r: PublicOfferDTO = {
			title: o['title'],
			photos: o["photos"].map((p: string) => this.pb.files.getUrl(o, p, { 'thumb': '180x180' })),
			id: o.id,
		}
